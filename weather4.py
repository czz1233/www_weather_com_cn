...
#            re.findall(re.compile('(alarm.*?=)'),self.warning)[0]
        except (IndexError,requests.exceptions.RequestException,socket.timeout):
            time.sleep(2)
    ...
    def get_sql(self):
        ...
        pd.io.sql.to_sql(self.weather_csv,'weather_com_cn_test',conn,if_exists='append',
...
if __name__ == '__main__':
    ...
    def run():
        ...
        for i in l:
            w.connect_weather(i)
            w.connect_warning(i)
            if w.warning !={} and w.bs != 'zhan_wei_fu':                
                w.get_data()
#                print(i)
            else:
                pass
    ...
    while True:
        ...
        run()
        print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))+' -- 结束\n\n')
        time.sleep(3600)
#    w.get_csv()
#    df = w.weather_csv
